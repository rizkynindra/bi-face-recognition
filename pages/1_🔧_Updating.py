...
st.write("Silahkan update database disini")

menu = ["Adding","Deleting", "Adjusting"]
choice = st.sidebar.selectbox("Options",menu)
if choice == "Adding":
    name = st.text_input("Nama Lengkap",placeholder='Enter name')
    id = st.text_input("No KTP",placeholder='Enter id')
    ...
    def del_btn_callback(id):
        deleteOne(id)
        st.success("Information deleted")
    ...
    if submit_btn:
        ...
        if name == None and image == None:
            st.error("Information ID does not exist")
        else:
            st.success(f"Name of customer with ID {id} is: {name}")
            st.warning("Please check the image below to make sure you are deleting the right customer")
            st.image(image)
    ...
    if submit_btn:
        ...
        if old_name == None and old_image == None:
            ...
            with st.form(key='my_form'):
                st.title("Adjusting customer info")
